...
def afficher_resultat(item, num=1):
    print(f"\n┌── Résultat #{num} ──")
    champs_principaux = ['nom_famille','prenom','pseudo','email','telephone','adresse','ville','code_postal','pays','date_naissance','iban','phone','societe','siret','bic','nir','mobile','civilite']
...
def export_pdf(fiche, filename=None):
    ...
    for i, res in enumerate(fiche['resultats'], 1):
        ...
        pdf.ln(2)
    pdf.output(filename or f"rapport_{fiche['identite']['nom']}_{fiche['identite']['prenom']}.pdf")
